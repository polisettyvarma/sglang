...
def _fused_moe_kernel_sequence(
    ...
    *,
    ...
    gemm_block_shape: Optional[List[int]] = block_shape
...
def fused_experts_impl(
    ...
    # Auto-detect MXFP4: packed FP4 weights are stored as uint8 or int8 (two FP4
    # nibbles per byte). Detect by checking if scales are E8M0 (uint8) which is
    # unique to MXFP4. FP8/INT8 use float32 scales, INT4 uses float scales.
    use_mxfp4_w4a16 = (
        (w1.dtype == torch.uint8 or w1.dtype == torch.int8)
    ...
    if use_int4_w4a16:
        ...
        # MXFP4 packs 2 FP4 elements per byte along K -> w1.shape[2] = K // 2.
        assert (
            hidden_states.shape[1] == w1.shape[2] * 2
        ), "Hidden size mismatch (MXFP4 packed weights)"
        assert (w1.dtype == torch.uint8 or w1.dtype == torch.int8) and (
            w2.dtype == torch.uint8 or w2.dtype == torch.int8
        ), "MXFP4 expects packed uint8 or int8 weights"
        assert (
            w1_scale is not None and w2_scale is not None
        ), "MXFP4 requires E8M0 scales for both w1 and w2"
        # Note: block_shape may be passed in but is ignored for MXFP4 (uses fixed MX group size of 32)
    else:
        assert (
